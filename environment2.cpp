...
bool environment :: addtype(int type)
{
  ...
  if(CheckNumber(user_response))
  {
    ...
    for(int i = 0; i < stoi(user_response); i++)Addnew(type);
...
void environment :: Addnew(int identity)
{
...
void environment :: compete()
{
  ...
  for(numleft; numleft > 1 ; numleft = numleft -2 )
  {
    ...
    int num2 = num1;
    while(num2 == num1) num2 = rand() % numleft + 1;
    ...
    if(response1)
    {
      if(response2)
      {
        Addnew(pro1 -> identity);
        // total[pro1 -> identity]++;
        Addnew(pro2 -> identity);
        // total[pro2 -> identity]++;
        totalPrograms +=2;
      }
      else
